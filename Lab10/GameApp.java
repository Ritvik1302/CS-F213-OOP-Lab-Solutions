...
class Dealer implements Runnable {
	...
	/*
	 * **** DO NOT MODIFY **** this label is used by the dealer to set the game
	 * status
	 */
	...
	public void run() {
	    synchronized(gameData.lock1) { // STEP-1: write code to take a lock on gameData using lock1

	        // dealer executes until either (or both) players declare success 
	        while (!gameData.playerSuccessFlag[0] && !gameData.playerSuccessFlag[1]) { // STEP-2: specify condition for player1 and specify condition for player2
	            try {
	                ...
	                synchronized(gameData.lock2) { // STEP-3: write code to take a lock on gameData using lock2 and wait while no number has been pressed by the user on the GUI
	                ...
	                // STEP-4: initialize the announcedNumber in GameStat with the number pressed on GameGUI for the players to read
	                ...
	                // STEP-7: notify all the players waiting for the number to be announced by the dealer using lock1 of GameData
	                gameData.lock1.notifyAll();

	                // STEP-8: wait using lock1 of GameData while the players haven't checked the numbers
	        ...
	        if (gameData.playerSuccessFlag[0]) { // STEP-9: specify condition
	            lblGameStatus.setText("PLAYER-1 HAS WON"); // UNCOMMENT THE ONE LINE OF CODE WRITTEN BELOW AFTER SPECIFYING THE CONDITION
	        }
	        // Check if Player2 has won
	        if (gameData.playerSuccessFlag[1]) { // STEP-10: specify condition
	            lblGameStatus.setText("PLAYER-2 HAS WON"); // UNCOMMENT THE ONE LINE OF CODE WRITTEN BELOW AFTER SPECIFYING THE CONDITION
	        }
	        // Check if both Player1 and Player2 have won
	        if (gameData.playerSuccessFlag[0] && gameData.playerSuccessFlag[1]) { // STEP-11: specify condition
	            lblGameStatus.setText("BOTH PLAYER-1 AND PLAYER-2 HAVE WON"); // UNCOMMENT THE ONE LINE OF CODE WRITTEN BELOW AFTER SPECIFYING THE CONDITION
	        }

	        // UNCOMMENT THE FOLLOWING TWO LINES OF CODE ONCE YOU FINISH WRITTING CODE FOR ALL STEPS ABOVE
	...
	public void run() {
		...
		synchronized (gameData.lock1) {
			...
			while (gameData.gameCompleteFlag == false) {
				...
				while (gameData.noAnnouncedFlag == false || gameData.playerChanceFlag[id]) {
					try {
						gameData.lock1.wait();
					} catch (InterruptedException e) {
				...
				if (!gameData.gameCompleteFlag) {
					...
					// and set the back ground color of the button to GREEN using the following
					// statement
					// this.btnOnTicket[i].setBackground(Color.GREEN)
					for (int i = 0; i < MAXNO; i++) {
						if (ticket[i] == gameData.announcedNumber) {
							totalNumbersFound++;
							btnOnTicket[i].setBackground(Color.GREEN);
							break;
						}
					}
...
/****
 * THE INSTANCE OF THIS CLASS IS USED AS A MEANS OF COMMUNICATION * AND
 * SYNCHRONIZTION BETWEEN THE PLAYER AND DEALER THREADS ********** DONOT MODIFY
 * THE CODE
 ****/
class GameData {
